...
only look at pixels: the WebSocket bridge bakes a secret into the bundle, so it
can never be turned on for a public deploy, and the dev-only globals a script
might have reached for do not exist in a production build.

### Added
- **WebMCP: the editor's tools, published to in-page agents (`src/lib/webmcp/`).** The same curated ten-tool set the desktop bridge advertises — four readers, `get_screenshot`, `batch_design`, `set_variables` and the three static guideline tools — is now registered on the page's model context, discoverable with schemas and risk annotations, and callable without any transport or token. Execution routes through `executeToolCall`, the identical funnel chat and both MCP bridges use, so no handler logic is duplicated and every existing guard still applies. Nothing consequential is exposed: `publish_to_showcase` in particular is not, and a contract test fails if it drifts in.
- **The API itself is polyfilled, deliberately.** No stable browser ships WebMCP (Chrome 152 exposes neither `document.modelContext` nor `navigator.modelContext`) and Electron lags Chrome by definition, so waiting for it would have meant shipping nothing. The polyfill installs *only* when the browser has none, and every caller goes through `getModelContext()`, which prefers the native object — when Chrome ships the API, the polyfill stands down and the registration code is untouched. It mirrors the native API's awkward parts rather than improving on them (arguments as a JSON string; a handler's message replaced by a generic failure), because a client that is allowed to depend on our conveniences would break against the real thing.
- **Schemas ship in the bundle, pinned to the backend by a contract test.** The frontend builds and runs without the backend checked out, so the JSON Schemas cannot be imported from it — which makes them a second copy of a backend-owned contract, exactly the thing that rots quietly. `webmcpContract.test.ts` imports the sibling checkout the way `toolContract.test.ts` already does and asserts the copy stays *tighter*, never looser: no property the backend does not declare, nothing required that it does not accept.

### Changed
- **Mutating tool calls are serialized across every surface, not per transport.** The scene-writing handlers build from a snapshot and commit a whole replacement `nodesById`/`childrenById`/`rootIds`, so two in flight discard each other's work. That was previously prevented by arrangement — one queue per transport, and the two bridges never run together — which WebMCP breaks by being always on next to whichever bridge is live. `runToolCall` (`src/lib/toolCallQueue.ts`) now sits *inside* `executeToolCall`, so chat, both bridges and WebMCP inherit the guarantee by construction rather than by convention; plugins call it explicitly, being the one path that reaches the handlers directly. Read-only tools are exempt, so a backgrounded tab's `get_screenshot` — where `rAF` never fires and the call burns its whole timeout — cannot stall unrelated traffic.
- **A failed WebMCP call comes back as a result, not a rejection.** A rejection loses its message to the generic "Tool invocation failed" that the layer substitutes, which left an agent unable to tell "this canvas is read-only" from "your arguments are malformed" from "your script has an error on line 12". Tool-level failures now return `{isError: true, error, …}` — MCP's own convention — carrying the handler's own fields, which is how `batch_design`'s resume point survives to a caller that wants to continue a long script rather than restart it.

### Security
- **Editing tools are withheld on a canvas that is not the user's to edit.** `sharedViewStore` has always been explicit that the tool handlers mutate the scene *below* every UI-level guard, so the protection has been to remove the way to invoke them — and this release adds a new way. `batch_design` and `set_variables` are withheld at registration and refused again at call time; both are needed, because registration alone races the shared viewer's lazy mount, and refusal alone would advertise tools that always fail.
- **Reads of someone else's shared canvas are narrowed to what that viewer's screen can show.** This one needs no XSS: anyone able to send a `/c/:shareId` link could otherwise write instructions into hidden layers or embed source and have a stranger's agent ingest them as content to act on. Hidden nodes keep only id, type and name — the layers panel shows those anyway — and embed/component source HTML is dropped, since it is never rendered as text and is the densest hiding place in the format. Redaction is marked rather than silent, or the agent would report an embed as empty. Its limit is stated where it is implemented: this removes the invisible channels, not the inattentive ones.
- **A published tool's name cannot be silently taken over.** `registerTool` replaces by name, as remounts and hot reload require, so a claim token is now needed to replace a name the page already claimed. This is not a defence against script already running in the page — that can do worse directly — it exists because `getTools()` omits `execute`, which makes a substituted tool indistinguishable from the real one to the agent, the one party with no way to detect the swap.

### Fixed
- **`batch_get` search patterns can no longer hang the tab.** The tool compiled a caller-supplied regex and tested it against every node name, synchronously on the main thread; a catastrophic pattern such as `(a+)+$` froze the editor for the *user*, and nothing downstream could intervene — the tool-call timeout cannot preempt synchronous work. Patterns now compile through `namePattern.ts`, which refuses nested quantifiers and over-long patterns and reports the refusal as an actionable message. The check is a heuristic tuned to false-negative on purpose: the path is shared with the chat agent, and wrongly rejecting an ordinary layer search would be a worse everyday failure than missing an exotic hang. Two incidental faults on the same line went with it — an invalid regex threw out of the search loop instead of being reported, and the pattern was recompiled once per node.
