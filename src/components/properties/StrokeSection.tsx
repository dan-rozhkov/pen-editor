...
import {
  ...
  StackRowShell,
  useDragReorder,
} from "@/components/properties/stackRow";
 ...
 * layers. Its popover body and this component stay separate from
 * `FillSection`'s: strokes only support solid/linear/radial (no
 * image/pattern/video/style binding) and keep their own geometry section
 * below this component. The row shell itself (drag handle, trigger,
 * visibility/remove buttons) is shared via `StackRowShell`.
 */
        ...
        .map(({ paint, arrayIndex }) => {
          ...
          return (
            <StackRowShell
              ...
              drag={drag}
              visible={isVisible}
              onToggleVisible={() => commit(toggleFillVisibleAt(strokes, arrayIndex))}
              onRemove={() => commit(removeFillAt(strokes, arrayIndex))}
              itemLabel="stroke"
              triggerTitle="Edit stroke"
              triggerContent={
                <>
              ...
              popoverTitle={<span className="text-[11px] font-semibold text-text-primary">Stroke</span>}
            >
              <div className="flex items-center gap-1">
