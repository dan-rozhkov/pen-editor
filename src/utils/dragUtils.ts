...
import { resolveContentInsets } from './strokeInsets'
...
export function calculateDropPosition(
  ...
  // indicator box needs the same insets (resolveContentInsets) added back
  // in to line up with where children actually land inside a stroked frame.
  const contentInsets = resolveContentInsets(parentFrame)
  const paddingTop = contentInsets.top
  const paddingRight = contentInsets.right
  const paddingBottom = contentInsets.bottom
  const paddingLeft = contentInsets.left
