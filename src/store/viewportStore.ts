...
interface ViewportState {
  ...
  startSmoothZoom: (zoomDelta: number, centerX: number, centerY: number) => void
  stopAnimation: () => void
...
const EASING_FACTOR = 0.85      // Interpolation speed (higher = faster, max ~0.9)
...
export const useViewportStore = create<ViewportState>((set, get) => ({
  ...
  startSmoothZoom: (zoomDelta, centerX, centerY) => {
    ...
    const baseScale = state.animationFrameId !== null ? state.targetScale : state.scale
    const newTargetScale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, baseScale * zoomMultiplier))
    ...
    if (state.animationFrameId === null) {
      const tick = () => {
        ...
        if (relativeError < ZOOM_THRESHOLD) {
          ...
          set({ scale: targetScale, x: finalX, y: finalY, animationFrameId: null })
          return
        }
